...
{ 
	...
	HANDLE handleMutex;
	if (!IgnoreMutex) {
		handleMutex = CreateMutex(NULL, TRUE, L"Hyper11Fix-AlreadyRunning");
		if (GetLastError() == ERROR_ALREADY_EXISTS)
		{
			printf("Hyper11Fix is already running. Exiting this instance...\n");
			return 1;
		}
	}
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		if (ShowTrayIcon) {
			...
			NameAndVer = L"Hyper 11 Fix";
			NameAndVer2 = L"v" + ProgramVersion + L" Created by xbl4z3r.";
